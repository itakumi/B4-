...
def load_cifar10(batch=100):
    ...
    train_data = datasets.CIFAR10('./data', train=True, download=True, transform=transforms.Compose([ transforms.CenterCrop(random.randint(26,32)), transforms.RandomResizedCrop(32, scale=(0.08,1.0), ratio=(0.75,1.3)), transforms.RandomAffine([-30,30], scale=(0.8,1.2), shear=10), transforms.RandomHorizontalFlip(p=0.5), transforms.RandomPerspective(distortion_scale=0.5, p=0.5, interpolation=3), transforms.ToTensor(), transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])]))
