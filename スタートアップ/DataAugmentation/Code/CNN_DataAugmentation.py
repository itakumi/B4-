...
def load_cifar10(batch=100):
    ...
    #train_data = datasets.CIFAR10('./data', train=True, download=True, transform=transforms.Compose([ transforms.RandomResizedCrop(32, scale=(0.08,1.0), ratio=(0.75,1.3)), transforms.RandomHorizontalFlip(p=0.5), transforms.ToTensor(), transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])]))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if len(history['test_acc']) >= 2:
            if ((history['test_acc'][-1]*100) - (history['test_acc'][-2]*100)) < 0.5:
